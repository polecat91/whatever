<div class="notify text-center"></div>
<div class="container-fluid">
    <div class="row">
        <div class="col-xs-12 col-sm-6 col-md-4 col-lg-3 login-content">
            <div class="text-center">
                <h2>Login</h2>
                <!-- Main Form -->
                <div class="login-form-1">
                    <form class="text-left form-login" method="POST">
                        <div class="login-form-main-message"></div>
                        <div class="main-login-form">
                            <div class="login-group">
                                <div class="form-group">
                                    <label for="lg_username" class="sr-only">E-mail</label>
                                    <input type="text" class="form-control login-email" id="lg_username" required="required" placeholder="E-mail">
                                </div>
                                <div class="form-group">
                                    <label for="lg_password" class="sr-only">Password</label>
                                    <input type="password" class="form-control login-password" id="lg_password" required="required" placeholder="Password">
                                </div>
                            </div>
                            <button type="submit" class="login-button btn-primary text-center"><strong>Login</strong></button>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>